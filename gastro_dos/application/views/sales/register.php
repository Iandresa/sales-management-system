...
if (isset($warning))
{
	echo "<div id='warning_sale1' class='warning_mesage'>".$warning."</div>";
}

if (isset($daily_cash_completed))
{
	echo "<div id='warning_sale2' class='warning_mesage'>".$daily_cash_completed."</div>";
}

if (isset($cycle_cash_completed))
{
	echo "<div id='warning_sale3' class='warning_mesage'>".$cycle_cash_completed."</div>";
}
?>
...
<?php echo form_dropdown('mode',$modes,$mode,'onchange="$(\'#mode_form\').submit();"'); ?>
    ...
    <?php
        if(!$sale_id)
        {
            if($sub_cycle_is_completed)
            {    
        ?>
                <div style="float: right;">
                    <?php
                    echo anchor("sales/close_cycle_cash",
                        "<div class='small_button3'><span>".$this->lang->line('sales_close_cycle_cash')."</span></div>",
                        array('class'=>'thickbox none','title'=>$this->lang->line('sales_close_cycle_cash')));
                    ?>
                </div>
        <?php } 
            else
            {
                echo "<a id='bCloseCycleCash' title='".$this->lang->line('sales_close_cycle_cash')."'><div class='small_button3' style='float: right;'><span>".$this->lang->line('sales_close_cycle_cash')."</span></div></a>";
            }
        ?>
            <div style="float: right;">
                <?php
                echo anchor("sales/close_daily_cash",
                    "<div class='small_button3'><span>".$this->lang->line('sales_close_daily_cash')."</span></div>",
                    array('class'=>'thickbox none','title'=>$this->lang->line('sales_close_daily_cash')));
                ?>
            </div>
    <?php
        }
...
?>
    <div id="postpone_button" >
        ...
        <?php   
        if($this->session->userdata('customer_last_sale') == 'NULO') // ECP V_2 punto 3.7
        {
            if(isset($customer)) 
            {          
...
{ 
	...
	?>
		...
		<td>
                    <?php
                    if($this->session->userdata('customer_last_sale') == 'NULO') {
                        if($language=="english")
		...
		<td><?php
                if($this->session->userdata('customer_last_sale') == 'NULO') {
                  echo form_input(array('name'=>'discount','value'=>$item['discount'],'size'=>'3'));
                }
                else
                {
                    echo $item['discount'];
        ...
        if($this->session->userdata('customer_last_sale') == 'NULO') {
        echo form_submit("edit_item", $this->lang->line('sales_edit_item'));
        }
...
<div id="overall_sale">
	...
	{
	?>
	<?php if($this->session->userdata('customer_last_sale') == 'NULO') { ?> <!-- ECP V_2 punto 3.7 -->
 ...
 <!--fin Cancel_sale -->          
			...
			<div id="finish_sale">                           
                            <?php 
                                if($mode=='sale')
                                {
                                    ...
                                    { echo form_open("sales/complete_to_print/$sale_id",array('id'=>'complete_sale_form')); ?>
                                        <div class="small_button" id="fSaleC" style="float:left;margin-top:5px;"
                                            title="<?php echo $this->lang->line('sales_complete_sale'); ?>">
                                            <span>
                                                <?php
                                                echo $this->lang->line('sales_complete_sale');
                                                ?>
                                            </span>
                                        </div>
                                        </form>
                                ...
                                {
                                    echo form_open("sales/complete_to_print/$sale_id",array('id'=>'complete_return_form')); ?>
                                    <div class="small_button2" id="fReturn" style="float:left;margin-top:5px;"
                                         title="<?php echo $this->lang->line('sales_complete_return'); ?>">
                                            <span>
                                                <?php
                                                echo $this->lang->line('sales_complete_return');
                                                ?>
                                            </span>
                                    </div>
                                    </form>
                        ...
                        <?php if($mode=='sale' && !$hide_buttons) { ?> 
                            ...
                            <div id="unsettled_sale">
                                ...
                                echo anchor("sales/view_calendar/width:300/height:300",
                                    "<div class='calendar' style='float:right;margin-top:5px;'></div>",
                                    array('class'=>'thickbox none','title'=>$this->lang->line('sales_date_of_completion_order')));
                                ?>
                            </form>
                        </div>
                <?php   }  
		...
		{ ?>
                ...
                <?php if($this->Delivery->getSaleState($sale_id)=="postpone"){
                    ...
                    <div id="delivery_sale">
                        ...
                        echo anchor("sales/view_calendar/width:300/height:300",
                                    "<div class='calendar' style='float:right;margin-top:5px;'></div>",
                                    array('class'=>'thickbox none','title'=>$this->lang->line('sales_date_of_completion_delivery')));
                        ?>  
                    </form>
                    </div>
                <?php
		...
		?>
		<?php } ?><!-- ECP V_2 punto 3.7 -->
<!-- no mostrar hasta aquí -->
	...
	<?php if($this->session->userdata('customer_last_sale') == 'NULO') 
	{ ?> <!-- ECP V_2 punto 3.7 -->
	<div id="Payment_Types" >
		...
		<div style="height:100px;">
			<?php 
                          ...
                          echo form_open("sales/add_payment/$sale_id",array('id'=>'add_payment_form'));  
                         ?><!-- ECP V_2 punto 3.7 : en este caso mostrar normalmente si es NULO, de lo contrario, si se está cargando la última venta, mostrar tabla informativa-->
                    <table width="100%">
			<tr>
			<td>
	...
	else  //ECP V_2 punto 3.7 si cargo la última venta
	{ ?>
		<div id="Payment_Types" >
		...
		<table id="register">
			<thead>
					...
					<th style="width:66%;"><?php echo $this->lang->line('reports_type'); ?></th>
					<th style="width:23%;"><?php echo $this->lang->line('sales_amount'); ?></th>
...
$(document).ready(function()
{
    ...
    $("#customer").result(function(event, data, formatted)
    {
        ...
        <?php
        if ( (count($cart)==0) && ($mode=='sale') )//no puede haber una venta sinitems, por tanto si no se han seleccionado los items
        {
            $this->session->set_userdata('customer_last_sale', 'load_last_sale');
        }
        ?>  //ECP V_2 punto 3.7
    ...
    $("#fReturn").click(function()
    {
        if (confirm('<?php echo $this->lang->line("sales_confirm_finish_return"); ?>'))
        {
            $('#complete_return_form').submit();
        }
    });
    ...
    $("#bCloseCycleCash").click(function()
    {
        alert("<?php echo $this->lang->line("sales_cycle_cash_in_process"); ?>");
    });
    
    /*$("#bCloseDailyCash").click(function()
    {
        alert("<?php echo $this->lang->line("sales_daily_cash_in_process"); ?>");
    });*/
...
<script type="text/javascript" language="javascript">
// haces referencia al elemento (en este caso div) e indicas el tiempo de espera en segundos
var strCmd3 = "document.getElementById('warning_sale1').style.display = 'none'";
var waitseconds3 = 5;
...
<script type="text/javascript" language="javascript">
// haces referencia al elemento (en este caso div) e indicas el tiempo de espera en segundos
var strCmd3 = "document.getElementById('warning_sale2').style.display = 'none'";
var waitseconds3 = 5;
...
<script type="text/javascript" language="javascript">
// haces referencia al elemento (en este caso div) e indicas el tiempo de espera en segundos
var strCmd3 = "document.getElementById('warning_sale3').style.display = 'none'";
var waitseconds3 = 5;
