...
<div id="page_title" style="margin-bottom:8px;">
    <?php 
        if($mode=='receive')
            echo $this->lang->line('recvs_register'); 
        else
            echo $this->lang->line('recvs_returning'); 
    ?>
</div>
...
<?php
    if(isset($message))
    { 
        echo "<div id='success_sale' class='success_message'>".$message."</div>";
    } 
?>
...
<div id="register_wrapper">
	...
	<label id="item_label" for="item">
        <input type="hidden" name="directadd" id="directadd"/>    
	<?php
	if($mode=='receive')
	{
...
?>
		...
		<?php echo anchor("items/view/-1/1/width:360",
...
<div id="overall_sale">
	...
	<div id="finish_sale">
		...
		<?php echo form_textarea(array('name'=>'comment','id'=>'comment','value'=>'','rows'=>'4','cols'=>'23'));?>
		<br><br>
		...
		<?php
		    echo form_dropdown('payment_type',$payment_options, "","id='payment_type'");?>
        </td>
        </tr>
		...
		<?php
		    echo form_input(array('name'=>'amount_tendered','id'=>'amount_tendered','value'=>'','size'=>'10'));
		?>
        </td>
        </tr>
        ...
        <a href="" class="thickbox" id="showbill"></a>
                </div>
...
$(document).ready(function()
{
    ...
    $("#finish_sale_button").click(function()
    {
    	...
    	{
            $.post("<?php echo base_url();?>index.php/receivings/complete", {comment: $("#comment").val(), payment_type:$("#payment_type").val(), amount_tendered:$("#amount_tendered").val()}, function(data){
                $("#showbill").attr("href", "<?php echo base_url(); ?>index.php/receivings/receipt/" + data.id + "/"+ data.amount_tendered);
                $("#showbill").click();
