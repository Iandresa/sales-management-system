...
class Sale_lib
{
	...
	function get_cart()
	{
		...
		$cartTemp =  $this->CI->session->userdata('cart');
		$result = array();
		foreach($cartTemp as $item)
		{
			...
			elseif($this->CI->Item->exists($item['item_id']))
			{
				$result[] = $item;
			}
		}	
	...
	function set_cart($cart_data)
	{
        ...
        //new
        function get_cart_temp()
	{
		if(!$this->CI->session->userdata('cart_temp'))
			$this->set_temp_cart(array());

		//return $this->CI->session->userdata('cart');
		$cartTemp =  $this->CI->session->userdata('cart_temp');
		$result = array();
		foreach($cartTemp as $item)
		{
			if(!$item['item_id'])
        ...
        //new
        function set_temp_cart($cart_data)
	{
		$this->CI->session->set_userdata('cart_temp',$cart_data);
	}
        
        //new
        function get_temp_finished_cart()
	{
		if(!$this->CI->session->userdata('cart_temp_finished'))
			$this->set_temp_finished_cart(array());

		//return $this->CI->session->userdata('cart');
		$cartTemp =  $this->CI->session->userdata('cart_temp_finished');
		$result = array();
		foreach($cartTemp as $item)
		{
			if(!$item['item_id'])
    ...
    //new
    function set_temp_finished_cart($cart_data)
	{
		$this->CI->session->set_userdata('cart_temp_finished',$cart_data);
	}
        
    //new
    function get_finished_cart()
	{
		if(!$this->CI->session->userdata('cart_finished'))
			$this->set_finished_cart(array());

		//return $this->CI->session->userdata('cart');
		$cartFinished =  $this->CI->session->userdata('cart_finished');
		$result = array();
		foreach($cartFinished as $item)
		{
			if(!$item['item_id'])
			{
				$result[] = $item;
    ...
    //new
    function set_finished_cart($cart_data)
	{
		$this->CI->session->set_userdata('cart_finished',$cart_data);
	}
        
    //new
    function set_saleId($sale_id)
    {
        $this->CI->session->set_userdata('sale_id',$sale_id);
    }

    //new
    function set_hideButtons($condition)
	{
		$this->CI->session->set_userdata('hide_buttons',$condition);
	}
	...
	function empty_payments()
	{
        ...
        function empty_customer_last_sale()//ECP V_2 punto 3.7
	{
		$this->CI->session->unset_userdata('customer_last_sale');
	}
	...
	function set_mode($mode)
	{
        ...
        function get_dateFinish()
	{
		if(!$this->CI->session->userdata('sale_dateFinish'))
                    return date('Y-m-d H:i');

		return $this->CI->session->userdata('sale_dateFinish');
	}

	function set_dateFinish($dateFinish)
	{
		$this->CI->session->set_userdata('sale_dateFinish',$dateFinish);
	}
...
if($this->CI->session->userdata('customer_last_sale') == 'last_sale_loaded')//si está cargada la uktima venta del cliente, limpiar primero antes de add
{    
    $this->set_cart(array());
}	
...
//Get all items in the cart so far...
	...
	function out_of_stock($item_id)
	{
		...
		$quantity_added = $this->get_quantity_already_added($item_id);
		
		if ($item->quantity - $quantity_added < 0)
		{
			return true;
		}
	...
	function delete_item($line)
	{
        ...
        /*NEW*/
        function delete_item_finished($line)
	{
		$items=$this->get_finished_cart();		
                unset($items[$line]);
		$this->set_finished_cart($items);
	}
        /*****/
	...
	function empty_cart()
	{
        ...
        //new
        function empty_cartTemp()
	{
		$this->CI->session->unset_userdata('cart_temp');
	}
        
        //new
        function empty_cartTempFinished()
	{
		$this->CI->session->unset_userdata('cart_temp_finished');
	}
        
        //new
        function empty_cartFinished()
	{
		$this->CI->session->unset_userdata('cart_finished');
	}
        
    //new
    function empty_saleId()
	{
		$this->CI->session->unset_userdata('sale_id');
	}
        
    //new
    function empty_hideButtons()
	{
		$this->CI->session->unset_userdata('hide_buttons');
	}
    ...
    function clear_dateFinish()
	{
		$this->CI->session->unset_userdata('sale_dateFinish');
	}

	function clear_all()//se ejcuta por ejemplo cuado cambio de venta sa devoluciones
	{
        $this->clear_mode();
        $this->clear_dateFinish();
		$this->empty_cart();
        $this->empty_cartTemp();//new
        $this->empty_cartFinished();//new
        $this->empty_cartTempFinished();//new
        $this->empty_saleId();//new
        $this->empty_hideButtons();//new
		//Alain Multiple Payments
		$this->empty_payments();
		$this->delete_customer();
        $this->empty_customer_last_sale();//ECP V_2 punto 3.7
	}
	...
	function get_taxes()
	{
		...
		foreach($this->get_cart() as $line=>$item)
		{
			...
			foreach($tax_info as $tax)
			{
                ...
                foreach($this->get_finished_cart() as $line=>$item)
		{
			$tax_info = $this->CI->Item_taxes->get_info($item['item_id']);
	...
	function get_subtotal()
	{
		...
		foreach($this->get_cart() as $item)
		{
		    $subtotal+=($item['price']*$item['quantity']-$item['price']*$item['quantity']*$item['discount']/100);
                ...
                foreach($this->get_finished_cart() as $item)
		{
		    $subtotal+=($item['price']*$item['quantity']-$item['price']*$item['quantity']*$item['discount']/100);
		}
		return to_currency_no_money($subtotal);
