...
class Receiving extends Model
{
	...
	function save ($items,$supplier_id,$employee_id,$comment,$payment_type,$mode,$receiving_id=false)
	{
                $subsidaryID = $this->session->userdata('subsidary_id');//2013-02-04 HL
	...
	public function create_receivings_items_temp_table()
	{
        ...
        function get_receivings_finished_per_subsidary($subsidary_id)
        {
        ...
        function get_receivings_finished_per_id($receiving_id,$subsidary_id)
        {
           if(isset($subsidary_id))
           {
               $this->db->from('receivings');
               $this->db->where("receiving_id = $receiving_id and subsidary_id = $subsidary_id");
           }
        ...
        /*
	Get search suggestions to find receivings
	*/
	function get_receiving_search_suggestions($search,$limit=25)
	{
            ...
            $this->db->from('receivings');	
            $this->db->where('subsidary_id', $subsidaryID);//change migue
            $this->db->like('receiving_id', $search);
            $this->db->order_by("receiving_id", "asc");
            $by_receiving_id = $this->db->get();
            foreach($by_receiving_id->result() as $row)
            {
                $suggestions[] = $row->receiving_id.'|'.$row->receiving_id/*.' - '.$row->receiving_time*/;
            }
