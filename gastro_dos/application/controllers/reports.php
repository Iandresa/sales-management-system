...
{	
	...
	function date_input_excel_export($kind)
	{
	...
	function summary_sales($start_date, $end_date, $export_excel=0)
	{
		...
		$report_data2 = $model->getData2(array('start_date'=>$start_date, 'end_date'=>$end_date)); 
                
                
		$no_Data = false;
		if($report_data2->num_rows() == 0)
			$no_Data = true;
			
		foreach($report_data as $row)
		{                        
			$tabular_data[] = array($row['sale_date'],$row['count_sales'] ,to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']),to_currency($row['profit']),to_currency($row['cost']),anchor('sales/view_details/'.$row['sale_date'], "<div><span>".$this->lang->line('sales_details')."</span></div>", array('class'=>'thickbox none','title'=>$this->lang->line('sales_details'))));//ecp error en reporte "resumen de ventas"
		}
                
                
//                $report_dataReturn = $model->getDataReturn(array('start_date'=>$start_date, 'end_date'=>$end_date)); //ecp dividir ventas de devoluciones en reporte "resumen de ventas"
//                
//                foreach($report_dataReturn as $row)//ecp dividir ventas de devoluciones en reporte "resumen de ventas"
//		{                        
//			$tabular_data[] = array($row['sale_date'],-$model->getSummaryReturns($row['sale_date']) ,to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']),to_currency($row['profit']),to_currency($row['cost']),anchor('sales/view_details/'.$row['sale_date'], "<div><span>".$this->lang->line('sales_details')."</span></div>", array('class'=>'thickbox none','title'=>$this->lang->line('sales_details'))));//ecp error en reporte "resumen de ventas"
//		}
//			
		...
		$data = array(
			...
			"data" => $tabular_data,
			"summary_data" => get_report_footer(1,1,$model->getSummaryData(array('start_date'=>$start_date, 'end_date'=>$end_date))),
			"export_excel" => $export_excel,
			"report_data"=> $no_Data,
            "margin" => $this->margin_footer()
		);
	...
	function summary_categories($start_date, $end_date, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['category'], $row['quantity_purchased'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit']), to_currency($row['cost']));
		}
		...
		$data = array(
			...
			"headers" => $model->getDataColumns(),
			"data" => $tabular_data,
			"summary_data" => get_report_footer(1,0,$model->getSummaryData(array('start_date'=>$start_date, 'end_date'=>$end_date))),
			"export_excel" => $export_excel,
			"report_data"=> $no_Data,
                        "margin" => $this->margin_footer()
		);
	...
	{
		$this->load->model('reports/Summary_customers','Summary_customers');
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['customer']!= NULL?$row['customer']:'Ninguno',$row['count_sales'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit']), to_currency($row['cost']));//ECP visualizar venta con customer ninguno		    
                }
	...
	function summary_suppliers($start_date, $end_date, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
                    $tabular_data[] = array($row['supplier'], $row['quantity_purchased'], to_currency($row['subtotal']), to_currency($row['total']), /*to_currency($row['tax']),*/ to_currency($row['profit']), to_currency($row['cost']));
		}
	...
	function summary_items($start_date, $end_date, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array(character_limiter($row['name'], 16), $row['quantity_purchased'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit']), to_currency($row['cost']));
		}
	...
	//Summary employees report (reporte tabular de empleados)
	function summary_employees($start_date, $end_date, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['employee'], $row['count_sales'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit']), to_currency($row['cost']));
		}
                
//                $report_dataReturn = $model->getDataReturn(array('start_date'=>$start_date, 'end_date'=>$end_date)); //ecp dividir ventas de devoluciones en reporte "resumen de ventas"
//
//                foreach($report_dataReturn as $row)
//		{
//			$tabular_data[] = array($row['employee'], -$model->getSummaryReturns($row['employee_id']), to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['profit']), to_currency($row['cost']));
//		}
	...
	function summary_taxes($start_date, $end_date, $export_excel=0)
	{
		...
		$data = array(
			...
			"headers" => $model->getDataColumns(),
			"data" => $tabular_data,
			"summary_data" => get_report_footer(1,0,$model->getSummaryData2(array('start_date'=>$start_date, 'end_date'=>$end_date))),
			"export_excel" => $export_excel,
			"report_data"=> $no_Data,
                        "margin" => $this->margin_footer()
		);
	...
	function summary_discounts($start_date, $end_date, $export_excel=0)
	{
		...
		$data = array(
			...
			"headers" => $model->getDataColumns(),
			"data" => $tabular_data,
			"summary_data" => get_report_footer(0,1,$model->getSummaryData(array('start_date'=>$start_date, 'end_date'=>$end_date))),
			"export_excel" => $export_excel,
			"report_data"=> $no_Data,
                        "margin" => $this->margin_footer()
		);
	...
	function date_input($graph_header_name)
	{
		$data = $this->_get_common_report_data();
		$data['graph_header_name'] = $graph_header_name;
	...
	function graphical_summary_sales($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_graphical_summary_sales'),
	...
	function graphical_summary_sales_graph($start_date, $end_date)
	{
		$this->load->model('reports/Summary_sales');
		$model = $this->Summary_sales;
		$report_data = $model->getGraphicData(array('start_date'=>$start_date, 'end_date'=>$end_date));
		
                
                //ini ECP poner moneda en la leyenda "legend" del eje Y y_axis
                $this->load->model('reports/Subsidary');
                $model_Subsidary = $this->Subsidary;                
                $report_data_Subsidary = $this->Subsidary->get_info($this->session->userdata('subsidary_id'));                
                $report_data_currency = $this->Currency->get_info($report_data_Subsidary->currency_id);                
                $cur_symbol = $report_data_currency->cur_symbol;
                //fin ECP
		...
		$data = array(
			"title" => $this->lang->line('reports_sales_summary_report'),
			"yaxis_label"=>$this->lang->line('reports_revenue')."  ".$cur_symbol,
	...
	function graphical_summary_items($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_graphical_summary_items'),
	...
	function graphical_summary_items_graph($start_date, $end_date)
	{
		...
		$data = array(
			...
			"yaxis_label"=>$this->lang->line('reports_revenue')."  ".$cur_symbol,//ECP
			"data" => $graph_data
		);

		$this->load->view("reports/graphs/bar",$data);//ECP de line a bar
	}
	...
	function graphical_summary_categories($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_graphical_summary_categories'),
	...
	function graphical_summary_categories_graph($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_categories_summary_report'),
			"xaxis_label"=>$this->lang->line('reports_categories'),
			"yaxis_label"=>$this->lang->line('reports_revenue')."  ".$cur_symbol,//ECP
                        "data" => $graph_data
		);
	...
	function graphical_summary_suppliers($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_graphical_summary_suppliers'),
	...
	function graphical_summary_suppliers_graph($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_suppliers_summary_report'),
			"xaxis_label"=>$this->lang->line('reports_suppliers'),
			"yaxis_label"=>$this->lang->line('reports_expenses')."  ".$cur_symbol,//ECP
                        "data" => $graph_data
		);
	...
	function graphical_summary_employees($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_graphical_summary_employees'),
	...
	function graphical_summary_employees_graph($start_date, $end_date)
	{
		$this->load->model('reports/Summary_employees');
		$model = $this->Summary_employees;
		$report_data = $model->getGraph_Data(array('start_date'=>$start_date, 'end_date'=>$end_date));//ECP disjointing employee tabular and graphical reports
		...
		$data = array(
			"title" => $this->lang->line('reports_employees_summary_report'),
			"xaxis_label"=>$this->lang->line('reports_employees'),
			"yaxis_label"=>$this->lang->line('reports_revenue')."  ".$cur_symbol,//ECP
                        "data" => $graph_data
		);
	...
	function graphical_summary_taxes($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_graphical_summary_taxes'),
	...
	function graphical_summary_taxes_graph($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_taxes_summary_report'),
			"xaxis_label"=>$this->lang->line('reports_taxes'),
			"yaxis_label"=>$this->lang->line('reports_revenue')."  ".$cur_symbol,//ECP
                        "data" => $graph_data
		);
	...
	function graphical_summary_customers($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_graphical_summary_customers'),
	...
	function graphical_summary_customers_graph($start_date, $end_date)
	{
		$this->load->model('reports/Summary_customers');
		$model = $this->Summary_customers;
		$report_data = $model->getGraphicData(array('start_date'=>$start_date, 'end_date'=>$end_date));//ECP la consulta para el reporte gráfico ahora será != a la del reporte tabular
		...
		foreach($report_data as $row)
		{
		//ECP ini  2013-08-12 para visualizar nombre cliente Ninguno si no hay cliente seleccionado en una venta
                    $customer_name = 'Ninguno';
                    if($row['customer']) 
                        $customer_name = $row['customer'];
		    $graph_data[$customer_name] = $row['total'];
		//ECP fin
		}
	...
	function graphical_summary_discounts($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_graphical_summary_discounts'),
	...
	function graphical_summary_payments($start_date, $end_date)
	{
		...
		$data = array(
			"title" => $this->lang->line('reports_graphical_summary_payments'),
