...
class Items extends Secure_area implements iData_controller
{
    ...
    function view($item_id=-1, $use_0_inQuantity = false)
    {   
        /*HL (2013-03-15)*/
        if($item_id != -1)
        {   
        ...
        if ($use_0_inQuantity == 1)
            $data['use_0_inQuantity'] = true;
        ...
        foreach ($this->Item->get_all_for_sale()->result_array() as $row)
        {
        ...
        $data['selected_item'] = $this->Item->get_info($item_id)->owner_item_id;
        ...
        $data['editing'] = ($item_id == -1) ? false : true;
...
//------------------------------------------- Ramel
    ...
    function save($item_id=-1)
    {
        $item_data = array(
            ...
            'is_serialized' => $this->input->post('is_serialized'),
            'taxes_from_subsidary' => ($this->input->post('is_for_sale') == 0) ? $this->input->post('set_taxes') : 0,
            'is_forSale' => $this->input->post('is_for_sale'),
            'owner_item_id' => $this->input->post('item_id') == '' ? null : $this->input->post('item_id'),
            'quantity_to_use' => $this->input->post('quantity_to_use')
        );
        ...
        if ($this->Item->save($item_data, $item_id))
        {
            ...
            if($this->input->post('is_for_sale') == 1)//(HL 2013-04-12)
            {
...
//---------------------------------------------------------------------Ramel
        ...
        else
        {
            ...
            if ($rowCount > 2)
            {
                for ($i = 3; $i <= $rowCount; $i++)
                {
                    ...
                    /* $codigos = $this->Item->exists_code(); //oscar

                      //begin oscar
                      foreach($codigos as $it)
                      {
                      if($it->item_number == $item_number)
                      {
                      echo json_encode( array('success'=>false,'message'=>'Ya existe ese codigo, si no aparece es porque esta deshabilitado.') );
                      return;
                      }
                      }
                      //end oscar
                     */
                    if ($this->Item->save_import($item_data))
                    {
    ...
    function excel_help()
    {
        $this->load->view("items/excel_help", null);
    }

    function set_subsidary_taxes()
    {
        //$checked = $_POST['checked'];
        //$tax_name1 = $this->Appconfig->get('default_tax_1_name');
        //$tax_name2 = $this->Appconfig->get('default_tax_2_name');
        $tax_percent1 = $this->Appconfig->get('default_tax_1_rate');
        $tax_percent2 = $this->Appconfig->get('default_tax_2_rate');

        echo json_encode(array('success' => TRUE, 'tax_percent1' => $tax_percent1, 'tax_percent2' => $tax_percent2));
    }
    ...
    function check_item_name()
    {
        ...
        if( ($item_id != -1) && ($this->Item->item_name_equal($item_name,$item_id) == 1) )
            echo "no"; 
        else if($this->Item->check_item_name($item_name) == 0 )
            echo "no";
    }
