...
class Coffee extends Model
{
	function get_all()
	{
        ...
        /* (2013-07-24) Esto es viejo, ya no se hace asi. */
	...
	function get_all_unsettled_sale()
        {
            $subsidaryID = $this->session->userdata('subsidary_id');
                ...
                where (phppos_sales.state='order' or phppos_sales.state='postpone_order') and isDone = 0 and phppos_sales.subsidary_id = $subsidaryID
                order by sale_time ");
                    return $query;
        }
        
        function get_orders_to_finish()
        {
            $subsidaryID = $this->session->userdata('subsidary_id');
                ...
                left join phppos_customers ON phppos_customers.person_id = phppos_sales.customer_id
                left join phppos_people ON phppos_customers.person_id = phppos_people.person_id
                where (phppos_sales.state='order' or phppos_sales.state='postpone_order') and isDone = 0 and is_finished = 0 and phppos_sales.subsidary_id = $subsidaryID
                order by sale_time ");
                    return $query;
        }
        ...
        function get_all_unsettled_sale_by_saleID($saleID)
        {
            $subsidaryID = $this->session->userdata('subsidary_id');
                ...
                left join phppos_customers ON phppos_customers.person_id = phppos_sales.customer_id
                left join phppos_people ON phppos_customers.person_id = phppos_people.person_id
                where (phppos_sales.state='order' or phppos_sales.state='postpone_order') and isDone = 0 and phppos_sales.subsidary_id=$subsidaryID AND phppos_sales.sale_id=$saleID
                order by sale_time ");
                    return $query;
        }
        ...
        function  get_details_from_unsettled_sale2($saleID)
        {
            $query = $this->db->query("
                SELECT * FROM `phppos_sales_items` 
                join  phppos_items on phppos_sales_items.item_id = phppos_items.item_id 
                where sale_id = $saleID and is_itemFinished = 0");
                    return $query;
        }
        
        /*NEW*/
        function  get_details_from_postpone_sales($saleID)
        {
            $query = $this->db->query("
                SELECT * FROM `phppos_sales_items` 
                join  phppos_items on phppos_sales_items.item_id = phppos_items.item_id 
