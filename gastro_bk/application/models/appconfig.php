...
//		return $this->db->update('app_config',$config_data);			
	...
	function saveSub($key,$value,$subsidaryID)
	{				
		$this->db->where('subsidary_id',$subsidaryID);
		return	$this->db->update('subsidaries', array($key => $value));	
	}
	...
	function save_subsidary($data,$subsidary_id)//solo llama a metodos ya arreglados
	{
		$success=true;
		...
		foreach($data as $key=>$value)
		{
			if(!$this->saveSub($key,$value,$subsidary_id))
			{
				$success=false;
				break;
			}
