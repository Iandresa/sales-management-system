...
function get_subsidary_data_row($subsidary, $controller,$haveActions=TRUE)
{
	...
	$table_data_row.='<td width="20%">'.mailto($subsidary->email,character_limiter($subsidary->email,22)).'</td>';
	$table_data_row.='<td width="20%">'.$subsidary->phone.'</td>';	
	
	$table_data_row.='<td width="12%" align="left" colspan="2" style="font-weight:normal">'.anchor($controller_name."/view/$subsidary->subsidary_id/width:$width", $CI->lang->line('common_edit'),array('class'=>'thickbox','title'=>$CI->lang->line('subsidaries_update')));
	
	if($subsidary->subsidary_id != $CI->session->userdata('subsidary_id'))
		$table_data_row.='&nbsp;&nbsp;&nbsp;&nbsp;'.anchor($controller_name."/select/$subsidary->subsidary_id/width:$width", $CI->lang->line('common_select'),array('title'=>$CI->lang->line($controller_name.'_count')));
	...
	//$table_data_row.='&nbsp;&nbsp;&nbsp;&nbsp;'.anchor($controller_name."/delete_subsidary/$subsidary->subsidary_id/width:$width", $CI->lang->line('common_delete'),array('title'=>$CI->lang->line($controller_name.'_details_count'))).'</td>';	
...
function get_subsidary_data_row7C($subsidary, $controller)
{
	...
	$table_data_row.='<td colspan="2" width="12%">'.anchor($controller_name."/view/$subsidary->subsidary_id/width:$width", $CI->lang->line('common_edit'),array('class'=>'thickbox','title'=>$CI->lang->line('subsidaries_update')));
	...
	if( ($subsidary->subsidary_id != $CI->session->userdata('subsidary_id')) && !($CI->Enterprise->IsDeleted_Subsidary($subsidary->subsidary_id)) )//la actual no esta seleccionada
		$table_data_row.= '&nbsp;&nbsp;&nbsp;&nbsp;'.anchor($controller_name."/select/$subsidary->subsidary_id/width:$width", $CI->lang->line('common_select'),array('title'=>$CI->lang->line($controller_name.'_count')));//select
	...
	if($subsidary->subsidary_id != $CI->session->userdata('subsidary_id')) //no sale el eliminar de la que esta activada
	{
		...
		{					
				//recover
				$table_data_row.='&nbsp;&nbsp;&nbsp;&nbsp;'.anchor($controller_name."/UNdelete_subsidary_of_enterprise/$subsidary->subsidary_id/width:$width", "<span style=color:green>".$CI->lang->line('common_recover')."</span>",array('title'=>$CI->lang->line($controller_name.'_details_count')));
		}
		else
		{		
				//delete
				$table_data_row.='&nbsp;&nbsp;&nbsp;&nbsp;'.anchor($controller_name."/delete_subsidary_of_enterprise/$subsidary->subsidary_id/width:$width", "<span style=color:#666666;>".$CI->lang->line('common_delete')."</span>",array('title'=>$CI->lang->line($controller_name.'_details_count')));			
		}
	}
...
function get_enterprise_data_row($enterprise, $controller)
{
	...
	$table_data_row.='<td colspan="6" style="font-weight:bold;color:#4386A1">'.$enterprise->name.'</td>';	
